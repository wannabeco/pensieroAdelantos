...
class BaseDatosUsuarios extends CI_Model {
    ...
    {
        $this->db->select("u.*,u.estado as estadoU,p.nombrePerfil,e.nombre as nombreEmpresa,u.nombre as nombreUsuario,u.telefono as telUsuario,u.tipoDocumento as tipoDocumentoU,u.nroDocumento as nroDocumentoU,u.email as emailUsuario, u.celular as celUsuario, e.*,l.*");
        if(count($where) > 0)
        {
            $this->db->where($where);
        }
        ...
        $this->db->join($this->tableEmpresas." e","e.idempresa=u.idEmpresa","LEFT");
