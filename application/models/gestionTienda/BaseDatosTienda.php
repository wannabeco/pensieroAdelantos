...
class BaseDatosTienda extends CI_Model {
    ...
    public function __construct() 
    {
        ...
        $this->tablePedido               = "app_pedidos";
        $this->tableDetallePedido        = "app_detalle_pedido";
   ...
   //copear y pegar este query las veces que sea necesario realizar seleccion de datos en las tablas de la db, recordar cambiar el nombre
    ...
    public function infoEmpleado($where="")
    {
        $this->db->select("*");
        $this->db->where($where);
        $this->db->from($this->tableEmpleados." e");
        $this->db->join($this->tableEmpresas." em","em.idEmpresa=e.idEmpresa","INNER");
        $id = $this->db->get();
        //print_r($this->db->last_query());die();
        return $id->result_array();
    }  
    ...
    {
        $this->db->select("pre.fotoPresentacion as fotoProducto,p.*,pre.*,sub.*");
        $this->db->where($where);
    ...
    public function creaPedido($dataInserta)
    {
        $this->db->insert($this->tablePedido,$dataInserta);
        //print_r($this->db->last_query());die();
        return $this->db->insert_id();
    }
    public function creaDetPedido($dataInserta)
    {
        $this->db->insert($this->tableDetallePedido,$dataInserta);
        //print_r($this->db->last_query());die();
        return $this->db->insert_id();
    }
    public function actualizaEmpleado($where,$dataActualiza)
    {
        $this->db->where($where);
        $this->db->update($this->tableEmpleados,$dataActualiza);
        //print_r($this->db->last_query());die();
        return $this->db->affected_rows();
    }
    public function borraPedidoTemporal($where)
    {
        $this->db->delete($this->tablePedidoTemporal,$where);
        //print_r($this->db->last_query());die();
        return $this->db->affected_rows();
    }
    ...
    public function misPedidos($where=array())
    {
        ...
        $this->db->from($this->tablePedido." p");
