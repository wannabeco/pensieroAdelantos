...
project.controller('modulos', function($scope,$http,$q,constantes)
{
	...
	*/
	$scope.cargaPlantillaCreacionModulos = function(id,edita,idModulo)
	{
		$('#modalNModulo').modal("show");
		var controlador = 	$scope.config.apiUrl+"Admin/cargaPlantillaCreacionModulos";
		var parametros  = 	"edita="+edita+"&id="+id+"&padre="+$scope.padreModulo+"&idModulo="+idModulo;//quiere decir que va a crear uno nuevo
		constantes.consultaApi(controlador,parametros,function(json){
	...
	$scope.editarCategoria = function() {
		...
		if( $scope.categoriaEditar.nombreModulo != "") {
...
project.controller('modulosProcesador', function($scope,$http,$q,constantes)
{
	...
	$scope.procesaModulos = function(accion)
	{
		...
		var icono 			=	$("#iconoApp").val();
		//valido los campos
		if(nombreModulo == undefined || nombreModulo == "")
		{
			constantes.alerta("Atención","Debe escribir el nombre del módulo.","warning",function(){})
		}
		else if(nombreLargo == undefined || nombreLargo == "")
		{
			constantes.alerta("Atención","Debe el nombre completo del módulo.","warning",function(){})
		}
		...
		else
		{
			...
			if(accion == 1)//edita
			{
				constantes.confirmacion("Confirmación","Esta apunto de crear un nuevo módulo, desea continuar",'info',function(){
					...
					var parametros  = 	"edita="+accion+"&nombreModulo="+nombreModulo+"&icono="+icono+"&nombreLargo="+nombreLargo+"&urlModulo="+urlModulo+"&padre="+padre+"&idEditar="+idEditar+"&estado="+$("#estado").val();//quiere decir que va a crear uno nuevo
					constantes.consultaApi(controlador,parametros,function(json){
			...
			else//crea
			{
				...
				constantes.confirmacion("Confirmación","Esta apunto de crear un nuevo módulo, desea continuar",'info',function(){
					...
					var parametros  = 	"edita="+accion+"&nombreModulo="+nombreModulo+"&icono="+icono+"&nombreLargo="+nombreLargo+"&urlModulo="+urlModulo+"&padre="+padre;//quiere decir que va a crear uno nuevo
					constantes.consultaApi(controlador,parametros,function(json){
						if(json.continuar == 1)//procede a guardar los privilegios
						{
							//recorro e inserto los privilegios
